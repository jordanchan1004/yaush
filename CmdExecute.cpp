...
int cmd_execute(list<Command*>* command_list, const char* line_read, Job* fg_job, list<Job*>* bg_jobs)
{
	...
	for(int i=pipe_num-1;iter!=command_list->rend();iter++,i--)// run for all commands from tail to head
	{
		...
		else if(pid == 0)// child
		{
			...
			{
				int fid = open((*iter)->input, O_RDWR);
			...
			{
				int fid = open((*iter)->output, O_RDWR);
	...
	{
		fg_job = job;
		while(!(fg_job->pid_list).empty())// while pid list not empty
		{
			pid = wait(NULL);// wait for any child process to exit
			// search for this pid and remove it from list
			for(list<int>::iterator iter=(fg_job->pid_list).begin();iter!=(fg_job->pid_list).end();iter++)
			{
				if(*iter==pid)
				{
					(fg_job->pid_list).erase(iter);
					break;
				}
			}
		}
